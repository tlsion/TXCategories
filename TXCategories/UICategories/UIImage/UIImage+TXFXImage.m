...
@implementation UIImage (TXFXImage)

- (UIImage *)tx_imageCroppedToRect:(CGRect)rect
{
...
- (UIImage *)tx_imageScaledToSize:(CGSize)size
{   
...
- (UIImage *)tx_imageScaledToFitSize:(CGSize)size
{
    //calculate rect
    CGFloat aspect = self.size.width / self.size.height;
    if (size.width / aspect <= size.height)
    {
        return [self tx_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self tx_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}

- (UIImage *)tx_imageScaledToFillSize:(CGSize)size
{
    if (CGSizeEqualToSize(self.size, size))
    {
        return self;
...
- (UIImage *)tx_imageCroppedAndScaledToSize:(CGSize)size
...
- (UIImage *)tx_reflectedImageWithScale:(CGFloat)scale
{
	...
	//clip to gradient
	CGContextClipToMask(context, bounds, [[self class] tx_gradientMask]);
...
- (UIImage *)tx_imageWithReflectionWithScale:(CGFloat)scale gap:(CGFloat)gap alpha:(CGFloat)alpha
{
    //get reflected image
    UIImage *reflection = [self tx_reflectedImageWithScale:scale];
...
- (UIImage *)tx_imageWithShadowColor:(UIColor *)color offset:(CGSize)offset blur:(CGFloat)blur
{
    //get size
...
- (UIImage *)tx_imageWithCornerRadius:(CGFloat)radius
{
    //create drawing context
...
- (UIImage *)tx_imageWithAlpha:(CGFloat)alpha
{
    //create drawing context
	UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0f);
...
- (UIImage *)tx_imageWithMask:(UIImage *)maskImage;
{
    //create drawing context
    UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0f);
    CGContextRef context = UIGraphicsGetCurrentContext();
...
- (UIImage *)tx_maskImageFromImageAlpha
{
    //get dimensions
