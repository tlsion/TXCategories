...
@implementation UIImage(TXVector)
...
+ (UIImage *)tx_iconWithFont:(UIFont *)font named:(NSString *)iconNamed withTintColor:(UIColor *)tintColor clipToBounds:(BOOL)clipToBounds forSize:(CGFloat)fontSize{
    ...
    UIImage *image = [[self tx_cache] objectForKey:identifier];
    if(image == nil){
        ...
        if(!CGSizeEqualToSize(CGSizeZero, imageSize)){
            ...
            [[self tx_cache] setObject:image forKey:identifier];
        }
    }
    return image;
}

+ (UIImage *)tx_imageWithPDFNamed:(NSString *)pdfNamed forHeight:(CGFloat)height{
    return [self tx_imageWithPDFNamed:pdfNamed withTintColor:nil forHeight:height];
}

+ (UIImage *)tx_imageWithPDFNamed:(NSString *)pdfNamed withTintColor:(UIColor *)tintColor forHeight:(CGFloat)height{
    NSString *pdfFile = [[NSBundle mainBundle] pathForResource:pdfNamed ofType:@"pdf"];
    return [self tx_imageWithPDFFile:pdfFile withTintColor:tintColor forSize:CGSizeMake(MAXFLOAT, height)];
}

+ (UIImage *)tx_imageWithPDFFile:(NSString *)pdfFile withTintColor:(UIColor *)tintColor forSize:(CGSize)size{
