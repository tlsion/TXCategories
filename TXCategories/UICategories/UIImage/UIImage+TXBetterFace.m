...
@implementation UIImage (TXBetterFace)

- (UIImage *)tx_betterFaceImageForSize:(CGSize)size
                           accuracy:(TXAccuracy)accurary;
{
    NSArray *features = [UIImage _tx_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _tx_subImageForFaceFeatures:features
                                         size:size];
    }
}

- (UIImage *)_tx_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_tx_faceFeaturesInImage:(UIImage *)image accuracy:(TXAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == TXAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
