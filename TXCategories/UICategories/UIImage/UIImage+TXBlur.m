...
- (UIImage *)tx_lightImage
{
    ...
    return [self tx_blurredImageWithSize:CGSizeMake(60, 60) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)tx_extraLightImage
{
    ...
    return [self tx_blurredImageWithSize:CGSizeMake(40, 40) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)tx_tintedImageWithColor:(UIColor *)tintColor
{
    ...
    return [self tx_blurredImageWithSize:CGSizeMake(20, 20) tintColor:effectColor saturationDeltaFactor:-1.0 maskImage:nil];
}
...
{
    return [self tx_blurredImageWithSize:CGSizeMake(blurRadius, blurRadius)];
}
...
{
    return [self tx_blurredImageWithSize:blurSize tintColor:nil saturationDeltaFactor:1.0 maskImage:nil];
}
...
//| ----------------------------------------------------------------------------
- (UIImage *)tx_blurredImageWithSize:(CGSize)blurSize tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage
{
...
#define ENABLE_TINT                     1
    ...
    if (hasBlur || hasSaturationChange)
    {
        ...
        if (hasBlur)
        {
            CGFloat radiusX = [self tx_gaussianBlurRadiusWithBlurRadius:blurSize.width * inputImageScale];
            CGFloat radiusY = [self tx_gaussianBlurRadiusWithBlurRadius:blurSize.height * inputImageScale];
        ...
        if ( (effectCGImage = vImageCreateCGImageFromBuffer(inputBuffer, &format, &tx_cleanupBuffer, NULL, kvImageNoAllocate, NULL)) == NULL ) {
...
//
- (CGFloat)tx_gaussianBlurRadiusWithBlurRadius:(CGFloat)blurRadius
{
...
//
void tx_cleanupBuffer(void *userData, void *buf_data)
{ free(buf_data); }
