...
@implementation UIView (TXAnimation)

#pragma mark - Moves

- (void)tx_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self tx_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)tx_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self tx_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)tx_rotate:(int)degrees secs:(float)secs delegate:(id)delegate callback:(SEL)method {
    [UIView animateWithDuration:secs 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, tx_radiansForDegrees(degrees));
                     }
                     completion:^(BOOL finished) { 
                         if (delegate != nil) {
#pragma clang diagnostic push
...
- (void)tx_spinClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, tx_radiansForDegrees(90));
                     }
                     completion:^(BOOL finished) { 
                         [self tx_spinClockwise:secs];
                     }];
}
...
- (void)tx_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, tx_radiansForDegrees(270));
                     }
                     completion:^(BOOL finished) { 
                         [self tx_spinCounterClockwise:secs];
                     }];
}
...
- (void)tx_drainAway:(float)secs {
    self.tag = 20;
	[NSTimer scheduledTimerWithTimeInterval:secs/50 target:self selector:@selector(tx_drainTimer:) userInfo:nil repeats:YES];
}

- (void)tx_drainTimer:(NSTimer*)timer {
    ...
    [UIView animateWithDuration:secs/2 
                     ...
                     completion:^(BOOL finished) { 
                         [UIView animateWithDuration:secs/2 
                                          ...
                                          completion:^(BOOL finished) { 
                                              if (continuously) {
                                                  [self tx_pulse:secs continuously:continuously];
                                              }
                                          }];
                     }];
}
