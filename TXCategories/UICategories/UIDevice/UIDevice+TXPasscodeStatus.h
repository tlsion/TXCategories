...
typedef NS_ENUM(NSUInteger, TXPasscodeStatus){
    /* The passcode status was unknown */
    TXPasscodeStatusUnknown   = 0,
    /* The passcode is enabled */
    TXPasscodeStatusEnabled   = 1,
    /* The passcode is disabled */
    TXPasscodeStatusDisabled  = 2
};

@interface UIDevice (TXPasscodeStatus)

/**
 *  Determines if the device supports the `passcodeStatus` check. Passcode check is only supported on iOS 8.
 */
@property (readonly) BOOL tx_passcodeStatusSupported;
 ...
 */
@property (readonly) TXPasscodeStatus tx_passcodeStatus;
