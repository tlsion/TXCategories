...
@implementation UIDevice (TXHardware)
...
+ (NSString *)tx_platformString{
    NSString *platform = [self tx_platform];
...
+ (NSUInteger)tx_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)tx_cpuFrequency {
    return [self tx_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)tx_busFrequency {
    return [self tx_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)tx_ramSize {
    return [self tx_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)tx_cpuNumber {
    return [self tx_getSysInfo:HW_NCPU];
}


+ (NSUInteger)tx_totalMemoryBytes
{
    return [self tx_getSysInfo:HW_PHYSMEM];
}
