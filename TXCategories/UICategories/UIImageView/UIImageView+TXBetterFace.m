...
static CIDetector *tx_detector;

@implementation UIImageView (TXBetterFace)

void tx_hack_uiimageview_bf(){
    Method oriSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(setImage:));
    Method newSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(tx_setBetterFaceImage:));
    method_exchangeImplementations(newSetImgMethod, oriSetImgMethod);
}

- (void)tx_setBetterFaceImage:(UIImage *)image{
    [self setTx_needsBetterFace:image];
    if (![self tx_needsBetterFace]) {
        return;
    }
    
    [self tx_faceDetect:image];
}

char nbfKey;
- (void)setTx_needsBetterFace:(BOOL)needsBetterFace{
...
char detectorKey;
- (void)setTx_detector:(CIDetector *)detector{
    objc_setAssociatedObject(self,
                             &detectorKey,
                             detector,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)tx_faceDetect:(UIImage *)aImage
{
    ...
    dispatch_async(queue, ^{
        ...
        if (tx_detector == nil) {
            NSDictionary  *opts = [NSDictionary dictionaryWithObject:[self tx_fast] ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh
                                                              forKey:CIDetectorAccuracy];
            tx_detector = [CIDetector detectorOfType:CIDetectorTypeFace
                                                      context:nil
                                                      options:opts];
        }
        
        NSArray* features = [tx_detector featuresInImage:image];
        
        if ([features count] == 0) {
            NSLog(@"no faces");
            dispatch_async(dispatch_get_main_queue(), ^{
                [[self tx_imageLayer] removeFromSuperlayer];
            });
        } else {
            NSLog(@"succeed %lu faces", (unsigned long)[features count]);
            [self tx_markAfterFaceDetect:features
...
-(void)tx_markAfterFaceDetect:(NSArray *)features size:(CGSize)size{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        CALayer *layer = [self tx_imageLayer];
