...
static CIDetector * _tx_faceDetector;


@implementation UIImageView (TXFaceAwareFill)

+ (void)initialize
{
    _tx_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)tx_faceAwareFill {
    // Safe check!
    if (self.image == nil) {
        return;
    }
    
    CGRect facesRect = [self tx_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self tx_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) tx_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _tx_faceDetector;
...
- (void) tx_scaleImageFocusingOnRect:(CGRect) facesRect {
