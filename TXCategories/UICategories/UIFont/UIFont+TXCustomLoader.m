...
@implementation UIFont (TXCustomLoader)
 ...
 */
+ (BOOL) tx_deviceHasFullSupportForFontCollections {
 ...
 */
+ (BOOL)tx_registerFromURL:(NSURL *)fontURL {
...
/**
 ...
 @return Registration result
 */
+ (BOOL)tx_registerFromCGFont:(CGFontRef)fontRef {
...
+ (NSArray *)tx_registerFontFromURL:(NSURL *)fontURL {
    // Dictionary creation
    ...
    @synchronized(appRegisteredCustomFonts) {
        ...
        if (fontPSNames == nil) {
            ...
            // Check features
            if ([UIFont tx_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        if ([UIFont tx_registerFromURL:fontURL]) {
                    ...
                    if (loadedFont != NULL) {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
                            ...
                            if ([UIFont fontWithName:fontPSNames[0] size:kSizePlaceholder] == nil) {
                                ...
                                // If registration went ok
                                if ([UIFont tx_registerFromCGFont:loadedFont]) {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames
                                                                 forKey:fontURL];
...
+ (UIFont *) tx_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    NSArray *fontPSNames = [UIFont tx_registerFontFromURL:fontURL];
...
+ (UIFont *) tx_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension {
    // Get url for font resource
    NSURL *fontURL = [[[NSBundle mainBundle] URLForResource:name withExtension:extension] absoluteURL];
    
    return [UIFont tx_customFontWithURL:fontURL size:size];
}
