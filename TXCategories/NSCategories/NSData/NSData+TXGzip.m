...
@implementation NSData (TXGzip)

static void *tx_libzOpen()
{
    static void *libz;
...
{
    if (self.length == 0 || [self tx_isGzippedData])
    {
        return self;
    }
    
    void *libz = tx_libzOpen();
...
{
    return [self tx_gzippedDataWithCompressionLevel:-1.0f];
}
