...
@implementation NSString (TXNormalRegex)
#pragma mark - 正则相关
- (BOOL)tx_isValidateByRegex:(NSString *)regex{
...
//手机号分服务商
- (BOOL)tx_isMobileNumberClassification{
    /**
     * 手机号码
    ...
    if (([self tx_isValidateByRegex:CM])
        || ([self tx_isValidateByRegex:CU])
        || ([self tx_isValidateByRegex:CT])
        || ([self tx_isValidateByRegex:PHS]))
    {
        return YES;
    }
    else
...
//邮箱
- (BOOL)tx_isEmailAddress{
    NSString *emailRegex = @"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
    return [self tx_isValidateByRegex:emailRegex];
}

//身份证号
- (BOOL)tx_simpleVerifyIdentityCardNum
{
    NSString *regex2 = @"^(\\d{14}|\\d{17})(\\d|[xX])$";
    return [self tx_isValidateByRegex:regex2];
}
...
- (BOOL)tx_isCarNumber{
    ...
    return [self tx_isValidateByRegex:carRegex];
}

- (BOOL)tx_isMacAddress{
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self tx_isValidateByRegex:macAddRegex];
}

- (BOOL)tx_isValidUrl
{
    NSString *regex = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
    return [self tx_isValidateByRegex:regex];
}
...
#pragma mark - 算法相关
//精确的身份证号码有效性检测
+ (BOOL)tx_accurateVerifyIDCardNumber:(NSString *)value {
