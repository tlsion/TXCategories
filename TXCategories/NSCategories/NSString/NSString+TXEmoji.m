...
@implementation NSString (TXEmoji)

static NSDictionary * tx_s_unicodeToCheatCodes = nil;
static NSDictionary * tx_s_cheatCodesToUnicode = nil;

+ (void)tx_initializeEmojiCheatCodes
{
    ...
    @synchronized(self) {
        tx_s_unicodeToCheatCodes = forwardMap;
        tx_s_cheatCodesToUnicode = [reversedMap copy];
    }
}

- (NSString *)tx_stringByReplacingEmojiCheatCodesWithUnicode
{
    if (!tx_s_cheatCodesToUnicode) {
        [NSString tx_initializeEmojiCheatCodes];
    }
    
    if ([self rangeOfString:@":"].location != NSNotFound) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [tx_s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
...
- (NSString *)tx_stringByReplacingEmojiUnicodeWithCheatCodes
{
    ...
    if (self.length) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [tx_s_unicodeToCheatCodes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
