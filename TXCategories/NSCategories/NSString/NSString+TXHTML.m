...
static TX_HTMLEscapeMap tx_gAsciiHTMLEscapeMap[] = {
...
static TX_HTMLEscapeMap tx_gUnicodeHTMLEscapeMap[] = {
...
static int tx_EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
	const unichar *uchar = (const unichar*)ucharVoid;
	const TX_HTMLEscapeMap *map = (const TX_HTMLEscapeMap*)mapVoid;
	int val;
	if (*uchar > map->uchar) {
		val = 1;
...
@implementation NSString (TXHTML)
#pragma --mark GTM method

- (NSString *)tx_stringByEscapingHTMLUsingTable:(TX_HTMLEscapeMap*)table
...
//		_GTMDevLog(@"Unable to allocate buffer or data2");
	...
	for (NSUInteger i = 0; i < length; ++i) {
		TX_HTMLEscapeMap *val = bsearch(&buffer[i], table,
									 size / sizeof(TX_HTMLEscapeMap),
									 sizeof(TX_HTMLEscapeMap), tx_EscapeMapCompare);
...
- (NSString *)tx_stringByEscapingForHTML {
	return [self tx_stringByEscapingHTMLUsingTable:tx_gUnicodeHTMLEscapeMap
											 ofSize:sizeof(tx_gUnicodeHTMLEscapeMap) 
									escapingUnicode:NO];
} // gtm_stringByEscapingHTML

- (NSString *)tx_stringByEscapingForAsciiHTML {
	return [self tx_stringByEscapingHTMLUsingTable:tx_gAsciiHTMLEscapeMap
											 ofSize:sizeof(tx_gAsciiHTMLEscapeMap) 
									escapingUnicode:YES];
} // gtm_stringByEscapingAsciiHTML

- (NSString *)tx_stringByUnescapingFromHTML {
	...
	do {
		...
		if (length > 3 && length < 11) {
			if ([escapeString characterAtIndex:1] == '#') {
				...
				// "standard" sequences
				for (unsigned i = 0; i < sizeof(tx_gAsciiHTMLEscapeMap) / sizeof(TX_HTMLEscapeMap); ++i) {
					if ([escapeString isEqualToString:tx_gAsciiHTMLEscapeMap[i].escapeSequence]) {
						[finalString replaceCharactersInRange:escapeRange withString:[NSString stringWithCharacters:&tx_gAsciiHTMLEscapeMap[i].uchar length:1]];
						break;
					}
				}
			}
...
- (NSString *)tx_stringWithNewLinesAsBRs {
    @autoreleasepool {
        // Strange New lines:
        //	Next Line, U+0085
        //	Form Feed, U+000C
...
- (NSString *)tx_stringByRemovingNewLinesAndWhitespace {
    @autoreleasepool {
...
- (NSString *)tx_stringByLinkifyingURLs {
...
- (NSString *)tx_stringByStrippingTags {
    @autoreleasepool {
        ...
        finalString = [result tx_stringByRemovingNewLinesAndWhitespace];
...
- (NSString *)tx_stringByConvertingHTMLToPlainText {
    @autoreleasepool {
        // Character sets
        ...
        NSString *retString = [result tx_stringByUnescapingFromHTML];
