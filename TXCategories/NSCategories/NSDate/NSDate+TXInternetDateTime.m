...
@implementation NSDate (TXInternetDateTime)

// Instantiate single date formatter
+ (NSDateFormatter *)tx_internetDateTimeFormatter {
    @synchronized(self) {
...
+ (NSDate *)tx_dateFromInternetDateTimeString:(NSString *)dateString formatHint:(DateFormatHint)hint {
     // Keep dateString around a while (for thread-safety)
	NSDate *date = nil;
    if (dateString) {
        if (hint != DateFormatHintRFC3339) {
            // Try RFC822 first
            date = [NSDate tx_dateFromRFC822String:dateString];
            if (!date) date = [NSDate tx_dateFromRFC3339String:dateString];
        } else {
            // Try RFC3339 first
            date = [NSDate tx_dateFromRFC3339String:dateString];
            if (!date) date = [NSDate tx_dateFromRFC822String:dateString];
        }
    }
...
+ (NSDate *)tx_dateFromRFC822String:(NSString *)dateString {
    ...
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate tx_internetDateTimeFormatter];
