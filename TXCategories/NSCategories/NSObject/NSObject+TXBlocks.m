...
static inline dispatch_time_t tx_dTimeDelay(NSTimeInterval time) {
...
+ (id)tx_performBlock:(void (^)(void))block afterDelay:(NSTimeInterval)delay {
	...
	dispatch_after(tx_dTimeDelay(delay), dispatch_get_main_queue(), ^{  wrappingBlock(NO); });
...
+ (id)tx_performBlock:(void (^)(id arg))block withObject:(id)anObject afterDelay:(NSTimeInterval)delay {
	...
	dispatch_after(tx_dTimeDelay(delay), dispatch_get_main_queue(), ^{  wrappingBlock(NO, anObject); });
...
+ (void)tx_cancelPreviousPerformBlock:(id)aWrappingBlockHandle {
    [self tx_cancelBlock:aWrappingBlockHandle];
}
