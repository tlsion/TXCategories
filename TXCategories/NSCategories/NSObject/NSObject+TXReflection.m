...
@implementation NSObject (TXReflection)
...
{
    return [[self class] tx_propertyKeys];
}
+ (NSArray *)tx_propertyKeys {
...
{
    return [[self class] tx_propertiesInfo];
}
/**
...
+ (NSArray *)tx_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self tx_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)tx_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] tx_propertiesInfo];
...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] tx_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] tx_decodeType:returnType];
        NSString *encodeString = [[self class] tx_decodeType:encoding];
...
-(NSDictionary *)tx_protocolList{
    return [[self class]tx_protocolList];
}
+ (NSDictionary *)tx_protocolList
{
...
+ (NSArray *)tx_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] tx_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)tx_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)tx_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString tx_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
