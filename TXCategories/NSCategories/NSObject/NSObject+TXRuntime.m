...
BOOL tx_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void tx_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void tx_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (TXRuntime)

+ (void)tx_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    tx_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)tx_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    tx_method_append(self.class, klass, newMethod);
}

+ (void)tx_replaceMethod:(SEL)method fromClass:(Class)klass
{
    tx_method_replace(self.class, klass, method);
}

- (BOOL)tx_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class tx_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)tx_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)tx_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass tx_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)tx_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{
