...
@implementation NSObject (TXEasyCopy)

- (BOOL)tx_easyShallowCopy:(NSObject *)instance
{
...
- (BOOL)tx_easyDeepCopy:(NSObject *)instance
{
    Class currentClass = [self class];
    Class instanceClass = [instance class];
    ...
    while (instanceClass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyListCount; i++) {
            ...
            if (propertyName && !readonly) {
                ...
                BOOL flag = [NSObject tx_isNSObjectClass:propertyValueClass];
                if (flag) {
                    if ([propertyValue conformsToProtocol:@protocol(NSCopying)]) {
                        ...
                        [copyValue tx_easyDeepCopy:propertyValue];
                        [self setValue:copyValue forKey:propertyName];
                    }
                }else{
                    [self setValue:propertyValue forKey:propertyName];
...
+ (BOOL)tx_isNSObjectClass:(Class)clazz{
    ...
    if (flag) {
        ...
        if (!superClass) {
            ...
            return  [NSObject tx_isNSObjectClass:superClass];
        }
    }
}
@end
