...
static NSString *const TXAutocodingException = @"TXAutocodingException";
@implementation NSObject (TXAutoCoding)
+ (BOOL)supportsSecureCoding
{
    return YES;
}
+ (instancetype)tx_objectWithContentsOfFile:(NSString *)filePath
{
    //load the file
...
- (BOOL)tx_writeToFile:(NSString *)filePath atomically:(BOOL)useAuxiliaryFile
{
...
+ (NSDictionary *)tx_codableProperties
{
    //deprecated
...
- (NSDictionary *)tx_codableProperties
{
    ...
    if (!codableProperties)
    {
        ...
        {
            [(NSMutableDictionary *)codableProperties addEntriesFromDictionary:[subclass tx_codableProperties]];
...
- (NSDictionary *)tx_dictionaryRepresentation
{
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    for (__unsafe_unretained NSString *key in [self tx_codableProperties])
    {
...
- (void)tx_setWithCoder:(NSCoder *)aDecoder
{
    BOOL secureAvailable = [aDecoder respondsToSelector:@selector(decodeObjectOfClass:forKey:)];
    BOOL secureSupported = [[self class] supportsSecureCoding];
    NSDictionary *properties = [self tx_codableProperties];
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                [NSException raise:TXAutocodingException format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
            }
...
{
    [self tx_setWithCoder:aDecoder];
    return self;
}
#pragma clang diagnostic pop
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    for (NSString *key in [self tx_codableProperties])
    {
