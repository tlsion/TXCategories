...
+ (id)tx_doInstanceMethodTarget:(id)target selectorName:(NSString *)selectorName args:(NSArray *)args
{
    ...
    [invocation tx_setArgumentsWithArray:args];
...
- (void)tx_setArgumentsWithArray:(NSArray *)array
{
    if (nil != array && array.count > 0) {
        ...
        if (array.count != numargs) {
            ...
            [self tx_setArgumentWithObject:arg atIndex:0];
        }else{
            for (NSUInteger i = 0; i < numargs; i++) {
                [self tx_setArgumentWithObject:array[i] atIndex:i];
            }
        }
    }
}
